...
DESeq2::plotPCA(vsd, ntop = 100, intgroup = 'RADIATION.CLASS') + 
  ylim(-50, 50) + theme_bw() + geom_point(size=0.5)
```

## Differential expression analysis (only for BRCA)

```{r}
clinical_data.BRCA = clinical_data[clinical_data$`CANCER TYPE` == "BRCA", ]
sample_names.BRCA = sample_names[sample_names.formated %in% clinical_data.BRCA$SAMPLE]
countData = read.table("GSM1536837_06_01_15_TCGA_24.tumor_Rsubread_FeatureCounts_filtered.txt", header=TRUE, sep="\t", as.is=c(1,2))
countData.BRCA = countData[ , names(countData) %in% sample_names.BRCA]
head(countData.BRCA)
```

```{r}
row_names = countData$Gene
rownames(countData.BRCA) = row_names
head(countData.BRCA)
```
...
```{r}
library(ggplot2)
factor_columns = c("RADIATION RESPONSE", "RADIATION CLASS", 
                   "MOLECULAR PROFILING BEFORE OR AFTER RADIATION TREATMENT",
                   "RACE", "HISTOLOGY", "LOCATION", "PATHOLOGIC STAGE", 
                   "PATHOLOGIC T", "PATHOLOGIC N", "PATHOLOGIC M")

clinical_data.BRCA |> 
  ggplot(aes(`RADIATION RESPONSE`, fill = `RADIATION RESPONSE`)) + 
  geom_bar() + 
  ggtitle('Classes of RADIATION RESPONSE') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`RADIATION CLASS`, fill = `RADIATION CLASS`)) + 
  geom_bar() + 
  ggtitle('Classes of RADIATION CLASS') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`MOLECULAR PROFILING BEFORE OR AFTER RADIATION TREATMENT`, fill = `MOLECULAR PROFILING BEFORE OR AFTER RADIATION TREATMENT`)) + 
  geom_bar() + 
  ggtitle('Classes of MOLECULAR PROFILING') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
clinical_data.BRCA |> 
  ...
  geom_bar() + 
  ggtitle('Classes of RACE') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`HISTOLOGY`, fill = `HISTOLOGY`)) + 
  geom_bar() + 
  ggtitle('Classes of HISTOLOGY') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
clinical_data.BRCA |> 
  ...
  geom_bar() + 
  ggtitle('Classes of LOCATION') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) 
  #theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`PATHOLOGIC STAGE`, fill = `PATHOLOGIC STAGE`)) + 
  geom_bar() + 
  ggtitle('Classes of PATHOLOGIC STAGE') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`PATHOLOGIC T`, fill = `PATHOLOGIC T`)) + 
  geom_bar() + 
  ggtitle('Classes of PATHOLOGIC T') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`PATHOLOGIC N`, fill = `PATHOLOGIC N`)) + 
  geom_bar() + 
  ggtitle('Classes of PATHOLOGIC N') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

clinical_data.BRCA |> 
  ggplot(aes(`PATHOLOGIC M`, fill = `PATHOLOGIC M`)) + 
  geom_bar() + 
  ggtitle('Classes of PATHOLOGIC M') + 
  geom_text(aes(label = after_stat(count)), stat = 'count', vjust = 0) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
colData.BRCA = data.frame(clinical_data.BRCA[c("HISTOLOGY", "RADIATION CLASS", "RADIATION RESPONSE",
                                               "MOLECULAR PROFILING BEFORE OR AFTER RADIATION TREATMENT",
                                               "PATHOLOGIC STAGE", "RACE")])
rownames(colData.BRCA) = sample_names.BRCA
colData.BRCA
```

```{r}
methilation_data = read.table("Methylation_data_filtered.txt", header=TRUE, sep="\t", as.is=c(1,2))
head(methilation_data)
```
```{r}
colnames_methyl_data = colnames(methilation_data)[-1]
colnames_methyl_data
```
```{r}
colData.BRCA.filtered =  colData.BRCA[substr(rownames(colData.BRCA), 1, 15) %in% colnames_methyl_data, ]g
colData.BRCA.filtered
```
...
Data filtering: leave only two types of carcinoma
```{r}
histology_names = c("Infiltrating Ductal Carcinoma", "Infiltrating Lobular Carcinoma")
colData.BRCA.filtered =  colData.BRCA.filtered[colData.BRCA.filtered$HISTOLOGY %in% histology_names, , drop = FALSE]
colData.BRCA.filtered$HISTOLOGY = chartr(" ", "_", colData.BRCA.filtered$HISTOLOGY)
colData.BRCA.filtered
```
Additional filtering to reduce noise
```{r}
pathologic_stage = c("Stage I", "Stage II", "Stage III")
colData.BRCA.filtered =  colData.BRCA.filtered[colData.BRCA.filtered$MOLECULAR.PROFILING.BEFORE.OR.AFTER.RADIATION.TREATMENT == "After", ]
colData.BRCA.filtered =  colData.BRCA.filtered[colData.BRCA.filtered$RADIATION.RESPONSE == "Complete Response", ]
colData.BRCA.filtered$PATHOLOGIC.STAGE[colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IA" | colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IB"] = "Stage I"
colData.BRCA.filtered$PATHOLOGIC.STAGE[colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IIA" | colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IIB"] = "Stage II"
colData.BRCA.filtered$PATHOLOGIC.STAGE[colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IIIA" | 
                                         colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IIIB" | 
                                         colData.BRCA.filtered$PATHOLOGIC.STAGE == "Stage IIIC"] = "Stage III"
colData.BRCA.filtered =  colData.BRCA.filtered[colData.BRCA.filtered$PATHOLOGIC.STAGE %in% pathologic_stage, ]
na.omit(colData.BRCA.filtered)
colData.BRCA.filtered
```
...
```{r}
library(dplyr)
colData.BRCA.filtered %>% dplyr::count(HISTOLOGY)
```

Adjust samples to count data
```{r}
countData.BRCA.filtered = countData.BRCA[ , names(countData.BRCA) %in% rownames(colData.BRCA.filtered)]
head(countData.BRCA.filtered) 
```
...
library(stats)
#create a DESeq dataset object from the count matrix and the colData 
dds <- DESeqDataSetFromMatrix(countData = countData.BRCA.filtered, 
                              colData = colData.BRCA.filtered, 
                              design = as.formula(designFormula))
#print dds object to see the contents
print(dds)
```
...
#compute the contrast for the 'group' variable where 'Sensitive' 
#samples are used as the control group. 
DEresults = results(dds, contrast = c("HISTOLOGY", 'Infiltrating_Lobular_Carcinoma', 'Infiltrating_Ductal_Carcinoma'))
#DEresults = results(dds, contrast = c("RADIATION.CLASS", 'Sensitive', 'Resistant'))
...
```{r}
library(DESeq2)
DESeq2::plotMA(object = dds, ylim = c(-9, 9))
```
...
```{r}
d <- plotCounts(dds, gene=which.min(DEresults$padj), intgroup="HISTOLOGY", 
                returnData=TRUE)
library("ggplot2")
ggplot(d, aes(x=HISTOLOGY, y=count)) + 
...
vsd <- vst(dds)
#rld <- rlog(dds)
#pcaData = DESeq2::plotPCA(vsd, ntop = 500, intgroup = c('HISTOLOGY','RACE'), returnData = TRUE) 
pcaData = DESeq2::plotPCA(vsd, ntop = 100, intgroup = c('HISTOLOGY'), returnData = TRUE) 
DESeq2::plotPCA(vsd, ntop = 100, intgroup = 'HISTOLOGY') +  
  ylim(-50, 50) + theme_bw() + geom_point(size=0.5)
pcaData
```
...
ggplot(pcaData, aes(x= PC1, y = PC2))+
  geom_point(size= 3, aes(shape=HISTOLOGY, color=PATHOLOGIC.STAGE)) +
